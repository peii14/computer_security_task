...
def find_the_correctly_encrypted():
    with open('./secret_key.pem', 'r') as key_file:
        key = RSA.import_key(key_file.read())

    cipher = PKCS1_OAEP.new(key)

    for encrypted_msg in ciphers:
        try:
...
def find_the_real_code():
    ...
    for message in messages:
        try:
            ...
            hashFromSignature = pow(int.from_bytes(
                signature, byteorder='big'), pub_key.e, pub_key.n)
            if hashFromSignature == int.from_bytes(sha512(message['text']).digest(), byteorder='big'):
                return message['text']
        except ValueError as error:
            print('Not this one', error)
